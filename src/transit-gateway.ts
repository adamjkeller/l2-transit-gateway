...
abstract class TransitGatewayBase
  ...
  implements ITransitGateway
{
   ...
   */
  addRouteTable(id: string, tags?: cdk.Tag[]): TransitGatewayRouteTable {
...
export class TransitGateway extends TransitGatewayBase {
  ...
  constructor(scope: Construct, id: string, props: TransitGatewayProps = {}) {
    ...
    this.transitGatewayArn = this.getResourceArnAttribute(
      cdk.Stack.of(this).formatArn(transitGatewayArnComponents(resource.ref)),
      transitGatewayArnComponents(this.physicalName)
    );
  }
}

export function transitGatewayArnComponents(
  transitGatewayResourceName: string,
  region?: string
): cdk.ArnComponents {
  return {
